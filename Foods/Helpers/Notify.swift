...
class NotificationView: UIView {
    ...
    @objc public func present() {
        DispatchQueue.main.async {[self] in
            ...
            frame = CGRect(x: 0, y: -130, width: window.frame.width, height: 130)
            UIView.animate(withDuration: 0.5, delay: 0, usingSpringWithDamping: 0.5,
                           initialSpringVelocity: 0.5, options: .curveEaseIn) {
                self.frame = CGRect(x: 0, y: 0, width: self.frame.width, height: 130)
            }
    ...
    @objc private func handle(swipe: UISwipeGestureRecognizer) {
        ...
        #endif
        var offsetPoint: CGPoint = CGPoint(x: 0, y: -200)
        ...
        UIView.animate(withDuration: 1, delay: 0, usingSpringWithDamping: 0.5,
                       initialSpringVelocity: 0.5, options: .curveEaseIn) {
            self.frame = CGRect(x: offsetPoint.x, y: offsetPoint.y, width: self.frame.width, height: 130)
            self.completion?()
        } completion: { _ in
